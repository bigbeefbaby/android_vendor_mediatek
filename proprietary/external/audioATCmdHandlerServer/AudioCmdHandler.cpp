...
static int findOneNum(unsigned int value)
{
	int ret = 0;
	while(value)
	{
...
ACHStatus AudioCmdHandler::DLCustSPHMagiParamToNV(void *pParam)
{
    ...
    for (int i = 0; i < 32 ;  i++)
    {
        SLOGV("MAGI_CONFERENCE_STRUCT[%d] =%d", i, ((unsigned short *)pSPHMagiPara) + i);
    }
...
ACHStatus AudioCmdHandler::ULCustSPHMagiParamFromNV(void *pParam, int *len)
{
    ...
    for (int i = 0; i < 32 ;  i++)
    {
        ALOGD("MAGI_CONFERENCE_STRUCT[%d] =%d", i, ((unsigned short *)pSPHMagiPara) + i);
    }
...
ACHStatus AudioCmdHandler::DLCustSPHHACParamToNV(void *pParam)
{
    ...
    for (int i = 0; i < 32; i++)
    {
        ALOGD("HAC_STRUCT[%d] =%d", i, ((unsigned short *)pSPHHACPara) + i);
    }
	for (int i = 0; i < 270;  i++)
    {
        ALOGD("HAC_STRUCT[%d] =%d", i + 32, ((short *)pSPHHACPara) + i + 32);
    }
...
ACHStatus AudioCmdHandler::DLCustSPHLPBKParamToNV(void *pParam)
{
    int write_size = 0;
    int size = 0;
    ...
    for (int i = 0; i < 48; i++)
    {
        ALOGD("LPBK_STRUCT[%d] =%d", i, ((unsigned short *)pSPHLPBKPara) + i);
    }
...
ACHStatus AudioCmdHandler::ULCustSPHLPBKParamFromNV(void *pParam, int *len)
{
    int size = 0;
    int dataLen = 0;
    ...
    AUDIO_CUSTOM_SPEECH_LPBK_PARAM_STRUCT *pSPHLPBKPara = (AUDIO_CUSTOM_SPEECH_LPBK_PARAM_STRUCT *)malloc(sizeof(AUDIO_CUSTOM_SPEECH_LPBK_PARAM_STRUCT));
    if (pSPHLPBKPara == NULL || pParam == NULL)
    {
        return ACHFailed;
    }
...
ACHStatus AudioCmdHandler::DLCustSPHWBParamToNV(void *pParam, int block)
{
    ...
    if (block == 0)
    {
        ...
        for (int i = 0; i < SPH_ENHANCE_PARAM_NUM ; i++)
        {
            SLOGV("Received speech mode parameters sph_mode_wb_param[0][i]=%d", i, pCustomPara->sph_mode_wb_param[0][i]);
        }
	...
	{
        size = sizeof(AUD_SPH_AFTER_FOUR_MODE_PARAM_STRUCT) + sizeof(int);
        dataDecode((char *)pParam, size);
...
ACHStatus AudioCmdHandler::DLCustHFPParamToNV(void *pParam)
{
    int write_size = 0;
    int size = 0;
    ...
    for (int i = 0; i < 48; i++)
    {
        ALOGD("LPBK_STRUCT[%d] =%d", i, ((unsigned short *)pHFPPara) + i);
    }
...
ACHStatus AudioCmdHandler::ULCustHFPParamFromNV(void *pParam, int *len)
{
    int size = 0;
    int dataLen = 0;
...
{
    ALOGD("AudioCmdHandler::AudioDMNRTuning() in");
...
void AudioCmdHandler::printCustGainTable(AUDIO_GAIN_TABLE_STRUCT *gainTable, int block)
{
	...
	if(block == 1)
	{
		for(i=0; i<12; i++)
		{
			STREAM_GAIN *ptr = (STREAM_GAIN *)((void *)gainTable + i*sizeof(STREAM_GAIN));
			for(j=0; j<3; j++)
			{
...
ACHStatus AudioCmdHandler::getGainInfoForDisp(void *pParam, int *len, int block)
{
	...
	if(GainInfoFlag == false)
	{
		...
		status_t ret = AudioSystem::GetAudioData(AUDIO_GET_LGE_DISP, sizeof(PCDispTotolStru), (void *)&mDispStru);
		if(ret != NO_ERROR)
		{
...
ACHStatus AudioCmdHandler::ULCustGainParamFromNV(void *pParam, int *len, int block)
{
...
ACHStatus AudioCmdHandler::DLCustGainParamToNV(void *pParam, int block)
{
...
ACHStatus AudioCmdHandler::getBtNrecInfoForDisp(void *pParam, int *len)
{
	...
	//TBD:get PCDispTotolStru from Audio HAL
	status_t ret = AudioSystem::GetAudioData(AUDIO_GET_BT_NREC_DISP, sizeof(PCDispItem), (void *)&mBtNrecDispItem);
	if(ret != NO_ERROR)
	{
		ALOGE("read PCDispTotolStru error");
		return ACHFailed;
...
ACHStatus AudioCmdHandler::DLCustBtNrecParamToNV(void * pParam)
{
    int write_size = 0;
    int size = 0;
...
ACHStatus AudioCmdHandler::ULCustBtNrecParamFromNV(void * pParam,int * len)
{
    int size = 0;
    int dataLen = 0;
...
ACHStatus AudioCmdHandler::startRecorder(const char *filePath, int recordAudioSource)
{
    ...
    if (m_MediaRecorderClient.get() == NULL)
    {
		...
		ALOGD("m_MediaRecorderClient.get() is: 0x%x", m_MediaRecorderClient.get());
...
{
    ALOGD("AudioCmdHandler::PCMRecorderListener::notify-received message: msg=%d, ext1=%d, ext2=%d", msg, ext1, ext2);
    switch (msg)
    {

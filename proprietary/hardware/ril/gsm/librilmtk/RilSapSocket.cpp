...
void RilSapSocket::initSapSocket(const char *socketName,
    ...
    char socket_default_name[4][30] = {"sap_uim_socket1", "sap_uim_socket2", "sap_uim_socket3",
            "sap_uim_socket4"};
    ...
    if (strcmp(socketName, socket_name[0]) == 0) {
        ...
        if (strcmp(socketName, socket_name[2]) == 0) {
            if(!SocketExists(socketName)) {
        ...
        if (strcmp(socketName, socket_name[3]) == 0) {
            if(!SocketExists(socketName)) {
...
bool RilSapSocket::isCdmaLteDcSupport()
{

...
#define SOCKET_NAME_MBIM "rild-mbim"
#define SOCKET_NAME_MBIM_MD2 "rild-mbim-md2"
...
/* mbim start */
static int s_fdMBIM_listen = -1;
static int s_fdMBIM_command = -1;
/* mbim end */
...
/* mbim start */
static struct ril_event s_MBIMlisten_event;
static struct ril_event s_MBIMcommand_event;
/* mbim end */
...
static pthread_mutex_t s_worldPhoneMutex[SIM_COUNT] = {PTHREAD_MUTEX_INITIALIZER};
 ...
 * Define C2K only command's dispatch/response function here to avoid build error.
 */
...
/// M: ril proxy
static UnsolResponseInfo s_mtk_local_urc_commands[] = {
...
static void onNewCommandConnect(RIL_SOCKET_ID socket_id) {
    ...
    pthread_mutex_t* worldPhoneMutexHook = &s_worldPhoneMutex[socket_id];
    ...
    ret = pthread_mutex_lock(worldPhoneMutexHook);
    assert (ret == 0);
    ...
    ret = pthread_mutex_unlock(worldPhoneMutexHook);
    assert (ret == 0);
...
static void listenCallback (int fd, short flags, void *param) {
    ...
    if (!is_phone_socket && (NULL == sapSocket)) {
...
/* mbim start */
static void processMbimCommandsCallback(int fd, short flags, void *param) {
    RLOGD("[MBIM]processMbimCommandsCallback");
    ...
    while(recvLen == -1) {
        recvLen = recv(s_fdMBIM_command, buffer, MAX_DATA_SIZE, 0);
        if (recvLen == -1) {
            RLOGE("[MBIM] fail to receive data from ril-mbim socket. errno = %d", errno);
            if(errno != EAGAIN && errno != EINTR) {
                break;
            }
        }
    ...
    RLOGD("[MBIM] data receive from mbim is %s, data length is %d", buffer, recvLen);
    ...
    if (strcmp(buffer, "DISC") == 0 || recvLen <= 0) { //recLen == 0 means the client is disconnected.
        ...
        if(recvLen != -1) {
            if (!isDualTalkMode()) {
                sendLen = send(s_fdMBIM_command, buffer, recvLen, 0);
            }
            if (sendLen != recvLen) {
                RLOGE("[MBIM] lose data when send disc cfm to mbim. errno = %d", errno);
            }
        } else {
            RLOGE("[MBIM] close MBIM due to error = %d", errno);
        }

        ril_event_del(&s_MBIMcommand_event);
        close(s_fdMBIM_command);
        s_fdMBIM_command = -1;
    } else {
        // Do nothing
    }
...
static void mbim_Callback (int fd, short flags, void *param)
{
    RLOGD("[MBIM] enter mbim_Callback");
    int ret;

    struct sockaddr_un peeraddr;
    socklen_t socklen = sizeof (peeraddr);

    assert (s_fdMBIM_command< 0);
    assert (fd == s_fdMBIM_listen);

    s_fdMBIM_command = accept(s_fdMBIM_listen, (sockaddr *) &peeraddr, &socklen);

    if (s_fdMBIM_command < 0 ) {
        RLOGE("[MBIM] Error on accept() errno:%d", errno);
        /* start listening for new connections again */
        rilEventAddWakeup(&s_MBIMlisten_event);
        s_fdMBIM_command = -1;
        return;
    }
    ...
    ret = fcntl(s_fdMBIM_command, F_SETFL, O_NONBLOCK);

    if (ret < 0) {
        RLOGE ("[MBIM] Error setting O_NONBLOCK errno:%d", errno);
    }

    RLOGD("[MBIM] librilmtk: new rild-mbim connection");


    ril_event_set (&s_MBIMcommand_event, s_fdMBIM_command, true,
    processMbimCommandsCallback, NULL);

    rilEventAddWakeup (&s_MBIMcommand_event);
}
/* mbim end */
...
static void startListen(RIL_SOCKET_ID socket_id, SocketListenParam* socket_listen_p) {
    ...
    int ret;
    char socket_name[10];
    char socket_default_name[10];

    memset(socket_name, 0, sizeof(char)*10);
    memset(socket_default_name, 0, sizeof(char)*10);

    switch(socket_id) {
        case RIL_SOCKET_1:
            strncpy(socket_default_name, RIL_getRilSocketName(), 9);
            break;
        case RIL_SOCKET_2:
            strncpy(socket_default_name, SOCKET2_NAME_RIL, 9);
            break;
        case RIL_SOCKET_3:
            strncpy(socket_default_name, SOCKET3_NAME_RIL, 9);
            break;
        case RIL_SOCKET_4:
            strncpy(socket_default_name, SOCKET4_NAME_RIL, 9);
            break;
        default:
            RLOGE("Socket id is wrong!!");
            return;
    ...
    if(isCdmaLteDcSupport()) {
        strcpy(socket_name, "m");
        strcat(socket_name, socket_default_name);
    } else {
        strcpy(socket_name, socket_default_name);
    }
...
RIL_registerSocket (const RIL_RadioFunctionsSocket *callbacks) {
    ...
    if (s_fdMBIM_listen < 0) {
        RLOGE("Failed to get socket : %s errno:%d", rildmbim, errno);
        //exit(-1);
    }
    ret = listen(s_fdMBIM_listen, 4);

    if (ret < 0) {
        RLOGE("Failed to listen on ril MBIM socket '%d': %s",
             s_fdMBIM_listen, strerror(errno));
        //exit(-1);
    } else {
        ril_event_set (&s_MBIMlisten_event, s_fdMBIM_listen, true,
                    mbim_Callback, NULL);
...
// External SIM [End]
    ...
    if (isMalSupported()) {
        ...
        if (chipMode != 1) {
            ...
            RLOGD("MAL socket is set to rild-mal");
            property_set("ril.mal.socket","rild-mal");
        }
        /* add mal client */
    ...
    if(Init) {
        ...
        switch(socketType) {
            case RIL_SAP_SOCKET:
                ...
                strncpy(socket_default_name[0], "sap_uim_socket1", 29);
                if (SIM_COUNT >= 2) {
                    // RilSapSocket::initSapSocket("sap_uim_socket2", UimFuncs);
                    strncpy(socket_default_name[1], "sap_uim_socket2", 29);
                }
                if (SIM_COUNT >= 3) {
                    // RilSapSocket::initSapSocket("sap_uim_socket3", UimFuncs);
                    strncpy(socket_default_name[2], "sap_uim_socket3", 29);
                }
                if (SIM_COUNT >= 4) {
                    // RilSapSocket::initSapSocket("sap_uim_socket4", UimFuncs);
                    strncpy(socket_default_name[3], "sap_uim_socket4", 29);
                }
        }

        for (int i = 0; i < SIM_COUNT; i++) {
            if (isCdmaLteDcSupport()) {
                strcpy(socket_name[i], "m");
                strcat(socket_name[i], socket_default_name[i]);
            } else {
                strcpy(socket_name[i], socket_default_name[i]);
            }
...
RIL_onRequestComplete(RIL_Token t, RIL_Errno e, void *response, size_t responselen) {
   ...
   if(pRI->pCI->requestNumber == RIL_LOCAL_REQUEST_GET_SHARED_KEY
    ...
    /*RIL_REQUEST_OEM_HOOK_RAW for ATCI and non-C2K project start*/
    if (!isCdmaLteDcSupport()) {
...
processRadioState(RIL_RadioState newRadioState, RIL_SOCKET_ID socket_id) {
    ...
    if((newRadioState > RADIO_STATE_UNAVAILABLE) && (newRadioState < RADIO_STATE_ON)) {
        ...
        if(newVoiceRadioTech != voiceRadioTech) {
            ...
            /* M: Modify this URC only responsed by rat controller. */
                                ...
                                size_t datalen, RIL_SOCKET_ID socket_id)
{
    ...
    // MBIM start
    if (RIL_UNSOL_MBIM_RESPONSE == unsolResponse) {
        RIL_Data_Call_Response_MBIM* responseDataCallForMbim = (RIL_Data_Call_Response_MBIM*)data;
        RLOGD("[MBIM] data call response: status=%d, suggestedRetryTime=%d, interfaceId=%d, active=%d, mtuSize=%d, \
type=%s, ifname=%s, addresses=%s, dnses=%s, gateways=%s",
            responseDataCallForMbim->status, responseDataCallForMbim->suggestedRetryTime,
            responseDataCallForMbim->cid, responseDataCallForMbim->active,
            responseDataCallForMbim->mtuSize, responseDataCallForMbim->type,
            responseDataCallForMbim->ifname, responseDataCallForMbim->addresses,
            responseDataCallForMbim->dnses, responseDataCallForMbim->gateways);
        if (s_fdMBIM_command > 0) {
            RLOGD("[MBIM] data sent to mbim is '%d'", sizeof(RIL_Data_Call_Response_MBIM));
            unsigned int sendLen = send(s_fdMBIM_command, responseDataCallForMbim, sizeof(RIL_Data_Call_Response_MBIM), 0);
            if (sendLen != (int) sizeof(RIL_Data_Call_Response_MBIM)) {
                RLOGD("[MBIM]lose data when send to mbim.");
            }
        }
    }
    // MBIM end
...
*/
    switch(request) {
        ...
        case RIL_REQUEST_OEM_HOOK_RAW_REDIRECT: return "OEM_HOOK_RAW_REDIRECT";
        ...
        // ECC retry.
        case RIL_REQUEST_EMERGENCY_REDIAL: return "RIL_REQUEST_EMERGENCY_REDIAL";
        case RIL_REQUEST_EMERGENCY_SESSION_BEGIN: return "RIL_REQUEST_EMERGENCY_SESSION_BEGIN";
        case RIL_REQUEST_EMERGENCY_SESSION_END: return "RIL_REQUEST_EMERGENCY_SESSION_END";
        ...
        case RIL_REQUEST_OEM_HOOK_RAW_WITH_PROXY: return "RIL_REQUEST_OEM_HOOK_RAW_WITH_PROXY";
 ...
 */
static int responseGetNitzTime(Parcel &p, void *response, size_t responselen) {
    RLOGE("responseGetNitzTime not support in GSM !!");
    return 0;
}
/**
 * M: To fix build error after GSM/C2K RIL command integration end.

...
ifeq ($(strip $(MTK_MODEM_SUPPORT)),0)
  $(call dep-err-common, please set MTK_MODEM_SUPPORT=  instead of 0)
endif
#######################################################
...
############################################################
#for MTK_HEADER_SUPPORT,platform in blacklist need MTK_HEADER_SUPPORT=yes,others can be no or unidentify
mtkplatformblacklist:= MT6572 MT6582 MT6592 MT8127 MT6752 MT2601 MT8163
ifeq (mt6595, $(strip $(TARGET_BOARD_PLATFORM)))
    ifneq (yes, $(strip $(MTK_HEADER_SUPPORT)))
        $(call dep-err-common,Please trun on MTK_HEADER_SUPPORT when TARGET_BOARD_PLATFORM is $(TARGET_BOARD_PLATFORM))
    endif
endif

ifeq (mt6795,$(strip $(TARGET_BOARD_PLATFORM)))
    ifeq (yes, $(strip $(MTK_HEADER_SUPPORT)))
        $(call dep-err-common,Please trun off MTK_HEADER_SUPPORT when TARGET_BOARD_PLATFORM is $(TARGET_BOARD_PLATFORM))
    endif
endif

ifneq ($(MTK_PLATFORM),)
    ifneq (MT6795,$(strip $(MTK_PLATFORM)))
...
ifeq ($(strip $(MTK_ANT_SUPPORT)),yes)
  ifneq ($(strip $(MTK_COMBO_CHIP)), MT6630)
    ifneq ($(strip $(MTK_COMBO_CHIP)), CONSYS_6797)
      $(call dep-err-common,please turn off dep-err-common or set MTK_COMBO_CHIP as MTK6630/6797)
    endif
  endif
endif
...
ifeq (yes,$(strip $(MTK_WFC_SUPPORT)))
  ...
  ifneq (yes,$(strip $(MTK_VOLTE_SUPPORT)))
    $(call dep-err-ona-or-offb,MTK_VOLTE_SUPPORT,MTK_WFC_SUPPORT)
  endif
  ifneq (yes,$(strip $(MTK_EPDG_SUPPORT)))
    $(call dep-err-ona-or-offb,MTK_EPDG_SUPPORT,MTK_WFC_SUPPORT)
  endif
  ifneq (no,$(strip $(MTK_FLIGHT_MODE_POWER_OFF_MD)))
    ifneq (,$(strip $(MTK_FLIGHT_MODE_POWER_OFF_MD)))
      $(call dep-err-offa-or-offb,MTK_FLIGHT_MODE_POWER_OFF_MD,MTK_WFC_SUPPORT)
    endif
  endif
endif
